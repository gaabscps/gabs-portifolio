...
export const ServerAddressCTA = ({ server, links }: Props) => {
  ...
  return (
    <TerminalWindow
      filename={`connect (${server.address})`}
      label="join the server"
      command={`join play@${server.address}`}
      id="join"
      mb={10}
    ...
    >
      <Box pt={2}>
        ...
        <Flex gap={2} flexWrap="wrap" align="center" mb={links?.discord || links?.changelog || links?.github || links?.website || links?.wiki ? 5 : 0}>
          {editions.map((e) => (
            <Box
              key={e.label}
              as="span"
        ...
        {(links?.discord || links?.changelog || links?.github || links?.website || links?.wiki) && (
